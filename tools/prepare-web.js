...
const FILES=['index.html','trade-plan.html','app.js','bbma-dashboard-ui.js','trade-plan.js','live.js','shared-market-logic.js',
...
/* APK-only enhancement: inject the BBMA dashboard after app.js. Keeping source index.html unchanged
 * avoids disturbing the existing web/PWA navigation while the Android view is verified. */
const indexOut=path.join(out,'index.html');
if(fs.existsSync(indexOut)){
  let html=fs.readFileSync(indexOut,'utf8');
  if(!html.includes('bbma-dashboard-ui.js')){
