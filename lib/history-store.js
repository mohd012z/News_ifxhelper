...
function hasHash(file,hash){try{return fs.readFileSync(file,'utf8').split(/\r?\n/).some(line=>{if(!line)return false;try{return JSON.parse(line).contentHash===hash;}catch{return false;}});}catch{return false;}}
function append(kind, record){
 const observedAt=record.observedAt||new Date().toISOString();
 const row=Object.assign({schemaVersion:2,kind,observedAt,timestamp:Time.stamp(observedAt)},record);
 row.entityKey=row.entityKey||Id.eventKey(row);
 const stateForHash={...row}; delete stateForHash.observedAt; delete stateForHash.timestamp; delete stateForHash.id; delete stateForHash.contentHash;
 row.contentHash=Id.contentHash(stateForHash);
 row.id=row.id||idFor([kind,row.entityKey,row.contentHash]);
 const file=monthPath(kind,observedAt); if(hasHash(file,row.contentHash)) return Object.assign(row,{deduped:true});
