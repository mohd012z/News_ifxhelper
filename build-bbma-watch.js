...
(async()=>{
 ...
 const dash=D.build(frames,Object.fromEntries(Object.keys(frames).map(tf=>[tf,s.gaps.length?'NEAR':'EXACT'])));
 const out={schemaVersion:2,generatedAt:new Date().toISOString(),symbol:SYMBOL,provider:r.provider,httpMs:r.httpMs,source:{candles:r.candles.length,valid:clean.length,gaps:s.gaps.length,duplicates:s.duplicates.length},news,analysis,dashboard:dash};
 fs.mkdirSync(path.dirname(OUT),{recursive:true});fs.writeFileSync(OUT,JSON.stringify(out,null,2));console.log('BBMA watch',SYMBOL,news.state,OUT,Object.keys(analysis).join(','));
